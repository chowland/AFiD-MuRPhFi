...
## Dimensional system
The velocity field $`\bu`$ is determined by the Navier-Stokes equation subject to the Oberbeck-Boussinesq (OB) approximation such that density changes are related linearly to temperature changes and are negligible everywhere except the buoyancy term:
```math
\pd _t \bu + (\bu \cdot \boldsymbol{\nabla}) \bu = -\rho_0^{-1} \grad p + g\alpha T \mathbf{\hat{e}_g} + \nu \nabla^2 \bu .
```
The pressure $`p`$ is determined to make the velocity field divergence-free ($`\grad\cdot\bu=0`$), consistent with the OB approximation.
...
```math
[\boldsymbol{x}] = H, \quad [T] = \Delta T, \quad [\bu] = U_T = \sqrt{g\alpha H\Delta T}, \quad [t] = H/U_T, \quad [p] = \rho_0 U_T^2 .
```
...
```math
\pd_t \bu + (\bu \cdot \grad) \bu = - \grad p + T \mathbf{\hat{e}_g} + \sqrt{\frac{Pr}{Ra_T}}\nabla^2 \bu ,
```
```math
...
If salinity is the dominant scalar in the buoyancy, we set `FFscaleS = 1` and take the free-fall scale based on $`S`$, that is $`[\bu] = U_S = \sqrt{g\beta H \Delta S}`$.
Analogous to the case above, $`\Delta S`$ is the difference in salinity between the walls.
The dimensionless equations now read
```math
\pd_t \bu + (\bu \cdot \grad) \bu = - \grad p + (R_\rho T - S) \mathbf{\hat{e}_g} + \sqrt{\frac{Sc}{Ra_S}}\nabla^2 \bu ,
```
```math
\pd_t T + (\bu \cdot \grad) T = \frac{1}{\tau\sqrt{Ra_S Sc}} \nabla^2 T ,
```
```math
\pd_t S + (\bu \cdot \grad) S = \frac{1}{\sqrt{Ra_S Sc}} \nabla^2 S .
```
...
```
```math
\pd_t S + (\bu \cdot \grad) S = \frac{\tau}{\sqrt{Ra_T Pr}} \nabla^2 S .
```
...
```math
\pd_t \bu + (\bu \cdot \grad) \bu = - \grad p + T \mathbf{\hat{e}_g} + \sqrt{\frac{Pr}{Ra_T}}\left(\nabla^2 \bu - \frac{\phi \bu}{\eta}\right) ,
```
```math
\pd_t T + (\bu \cdot \grad) T = \frac{1}{\sqrt{Ra_T Pr}} \nabla^2 T + \mathcal{S}\partial_t \phi ,
```
where $`\eta = (\beta \eps)^2`$ is the volume penalty coefficient for a dimensionless diffusive interface thickness $`\eps`$ and $`\mathcal{S}=L/c_p\Delta T`$ is the Stefan number.
...
```math
\hat{\eps} \frac{5}{6} \frac{L}{c_p\kappa_T} \pd_t \phi - \Gamma \nabla^2 \phi = -\frac{1}{\hat{\eps}^2} \phi (1-\phi)(\Gamma (1-2\phi) + \hat{\eps} (T-T_m)) .
```
Here $`\hat{\eps}=H\eps`$ is the dimensional interface thickness, $`T_m`$ is the equilibrium melting temperature, and $`\Gamma`$ is the coefficient arising from the Gibbs-Thomson relationship linking the interface temperature to the curvature $`K`$ of the interface:
```math
...
```math
A = \frac{6}{5\eps} \frac{\kappa}{U_T H} \frac{c_p \Gamma}{LH}, \quad B = \frac{A}{\eps^2}, \quad C = \frac{H \Delta T}{\Gamma} .
```

In the absence of temperature variations, $`\phi`$ has a steady state solution of
```math
\phi = \frac{1}{2}\left(1 + \tanh \left(\frac{x-x_i}{2\eps}\right)\right) ,
```
...
```math
\hat{\eps} = 0.01 \mathrm{cm}, \quad \Gamma = 0.2 \mathrm{cm °C}, \quad \hat{\eta} = 0.1(\beta \hat{\eps})^2 = 2.275\times 10^{-5} \mathrm{cm^2} .
```
...
Scaling by the domain height gives $`\eps \approx 6.6\times 10^{-4}`$ and $`\eta \approx 10^{-7}`$.
...
```
The "diffuse interface thickness" discussed by Couston et al is related to our parameter $`\eps`$ by $`\delta = 4\eps`$.
The interface thickness $`\eps`$ is therefore equal to *half* the grid spacing stated, giving $`\eps \approx 8\times 10^{-4}`$.
The remaining (appropriately scaled) numerical parameters for the phase field they use are
```math
A = \frac{6}{5\mathcal{S}\sqrt{RaPr}} \approx 1.7 \times 10^{-3}, \quad B = \frac{A}{\eps^2} = \frac{A}{(8\times 10^{-4})^2} \approx 2656, \quad C = 1,

```
and the volume penalty prefactor used is
```math
\eta = (\beta \eps)^2 = 1.46 \times 10^{-6} .
```
...
```math
A = \frac{6}{5\mathcal{S}RePr} \approx 6\times 10^{-4}, \quad B = \frac{A}{\eps^2} \approx 937 .
```
