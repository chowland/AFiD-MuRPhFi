...
! #endif
      ...
      !================================================
      !-- Interpolation Coefficients for Coarse Sal ---
      !================================================
      !-- Set-up large (l) and small (s) arrays
      zzl(1:nzmr) = zmr(1:nzmr)
      zzl(0) = 2.d0*zzl(1) - zzl(2)
      ...
      xxl(-1) = 2.d0*xxl(0) - xxl(1)
      !xxs(2:nx+1) = xm(1:nx)  !Downshift required because of downshift in phi. 
      !xxs(1) = 2.d0*xxs(2) - xxs(3)
      xxs(1:nx) = xm(1:nx)
      xxs(0) = 2.d0*xxs(1) - xxs(2)

      
      !-- Get weights for gradient calculations
      !-- Now construct Hermite basis function in x | cxsalc | Used for salinity
      do ic=1,nxm
         icr = irangs(ic)-1 !icr=0
         dlc = xxl(icr+1)-xxl(icr)
         dlm = xxl(icr)-xxl(icr-1)
         dlp = xxl(icr+2)-xxl(icr+1)
         lxa = 1.d0/dlc
         lxm = (xxs(ic) - xxl(icr))*lxa !xxs(1) - xxl(0) .. want xm(1)-xmr(0)
         lxp = 1.d0 - lxm
         h00=(1.d0+2.d0*lxm)*lxp*lxp
         h10=lxm*lxp*lxp
         h01=(1.d0+2.d0*lxp)*lxm*lxm
      ...
      do jc=0,nym
         ...
         h01=(1.d0+2.d0*lyp)*lym*lym
         h11=-lyp*lym*lym
         cysalc(1,jc)=-h10*dlc*dlc/dlm/(dlc+dlm)
         cysalc(2,jc)=h00-h11*dlp/(dlp+dlc)+h10*(dlc-dlm)/dlm
         cysalc(3,jc)=h01+h10*dlm/(dlm+dlc)+h11*(dlp-dlc)/dlp
         cysalc(4,jc)=h11*dlc*dlc/dlp/(dlp+dlc)
      enddo
      !-- Now construct Hermite basis function in z | czsalc
      do kc=0,nzm
         kcr = krangs(kc)-1
         dlc = zzl(kcr+1)-zzl(kcr)
         dlm = zzl(kcr)-zzl(kcr-1)
         ...
         h01=(1.d0+2.d0*lzp)*lzm*lzm
         h11=-lzp*lzm*lzm
         czsalc(1,kc)=-h10*dlc*dlc/dlm/(dlc+dlm)
         czsalc(2,kc)=h00-h11*dlp/(dlp+dlc)+h10*(dlc-dlm)/dlm
         czsalc(3,kc)=h01+h10*dlm/(dlm+dlc)+h11*(dlp-dlc)/dlp
         czsalc(4,kc)=h11*dlc*dlc/dlp/(dlp+dlc)
      enddo
