...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
subroutine SolveImpEqnUpdate_VY_ibm
    use param
    use local_arrays, only: vy, rhs
    use decomp_2d, only: xstart,xend
    use ibm_param, only: ibmasky, disty
    implicit none
    ...
    do ic=xstart(3),xend(3)
        do jc=xstart(2),xend(2)
            do kc=1,nxm
                km = max(1,kc - 1)
                kp = min(kc + 1,nxm)
                if (ibmasky(kc,jc,ic) == 2) then ! Liquid phase
                    ackl_b = 1.0d0/(1.0d0 - ac3sk(kc)*betadx)
                    amkl(kc) = -am3sk(kc)*betadx*ackl_b
                    ackl(kc) = 1.d0
                    apkl(kc) = -ap3sk(kc)*betadx*ackl_b
                    fkl(kc) = rhs(kc,jc,ic)*ackl_b
                elseif (ibmasky(kc,jc,ic) == 0) then ! Solid phase
                    amkl(kc) = 0.d0
                    ackl(kc) = 1.d0
                    apkl(kc) = 0.d0
                    fkl(kc) = -vy(kc,jc,ic)
                elseif (ibmasky(kc,jc,ic) == 1) then ! Upper boundary points
                    amkl(kc) = 0.d0
                    ackl(kc) = 1.d0
                    apkl(kc) = -disty(n)
                    fkl(kc) = disty(n)*vy(kp,jc,ic) - vy(kc,jc,ic)
                    n = n + 1
                elseif (ibmasky(kc,jc,ic) == -1) then ! Lower boundary points
                    amkl(kc) = -disty(n)
                    ackl(kc) = 1.d0
                    apkl(kc) = 0.d0
                    fkl(kc) = disty(n)*vy(km,jc,ic) - vy(kc,jc,ic)
                    n = n + 1
                end if
            end do
            amkT = amkl(2:nxm)
    ...
    return
end subroutine SolveImpEqnUpdate_VY_ibm