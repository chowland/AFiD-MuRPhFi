...
subroutine mean_zplane
    use param, only: nx,multiRes_Temp
    use mpih
    use hdf5
    ...
    if(.not. multiRes_Temp) allocate(Tplane(1:nxm,xstart(2):xend(2)))
    ...
    if(.not. multiRes_Temp)call zmean(temp(1:nxm,xstart(2):xend(2),xstart(3):xend(3)), Tplane, comm, zrank)
    ...
    if (zrank==0) then
        ...
        varname='temp'
        if(.not. multiRes_Temp) call write_H5_plane(file_id, varname, Tplane, 'z')
        if (moist) then
            varname='qhum'
    ...
    if (multiRes_Temp) then
        ...
        call zmeanr(temp_fine(1:nxmr,xstartr(2):xendr(2),xstartr(3):xendr(3)), Tplane, comm, zrank)
...
subroutine mean_yplane
    ...
    if(.not. multiRes_Temp) allocate(Tplane(1:nxm,xstart(3):xend(3)))
    ...
    if(.not. multiRes_Temp) call ymean(temp(1:nxm,xstart(2):xend(2),xstart(3):xend(3)), Tplane, comm, yrank)
    ...
    if (yrank==0) then
        ...
        varname='temp'
        if(.not. multiRes_Temp) call write_H5_plane(file_id, varname, Tplane, 'y')
        if (moist) then
            varname='qhum'
    ...
    if (multiRes_Temp) then
        ...
        call ymeanr(temp_fine(1:nxmr,xstartr(2):xendr(2),xstartr(3):xendr(3)), Tplane, comm, yrank)
