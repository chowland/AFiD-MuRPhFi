...
subroutine Mkmov_zcut
    use param, only: nzm, nzmr,multiRes_Temp
    use mpih
    use hdf5
    ...
    if(ic.le.xend(3) .and. ic.ge.xstart(3)) then
        ...
        varname = 'temp'
        if(.not. multiRes_Temp)   call write_H5_plane(file_id, varname, temp(1:nxm, xstart(2):xend(2), ic), 'z')
    ...
    if (multiRes_Temp) then 
        ...
        if(ic.le.xendr(3) .and. ic.ge.xstartr(3)) then
            ...
            varname = 'temp'
            call write_H5_plane(file_id, varname, temp_fine(1:nxmr, xstartr(2):xendr(2), ic), 'z')
